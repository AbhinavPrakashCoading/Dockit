// Test the CRUD API for parsed documents
const fetch = require('node-fetch');

const BASE_URL = 'http://localhost:3000/api';

async function testCRUDOperations() {
  console.log('🧪 Testing Parsed Documents CRUD API');
  console.log('=====================================');

  try {
    // Test 1: Parse a document first (this should auto-save to DB)
    console.log('\n1️⃣ Testing text-to-JSON parsing with auto-save...');
...
Educational Certificates: PDF format, max 500KB per file, graduation degree, mandatory`,
    ...
    // Test 2: List all parsed documents
    console.log('\n2️⃣ Testing GET /api/parsed-documents...');
    const listResponse = await fetch(`${BASE_URL}/parsed-documents`);
    ...
    if (listResult.data && listResult.data.length > 0) {
      ...
      console.log('\n3️⃣ Testing GET /api/parsed-documents/[id]...');
      const getResponse = await fetch(`${BASE_URL}/parsed-documents/${firstDoc.id}`);
      ...
      console.log('\n4️⃣ Testing PUT /api/parsed-documents/[id]...');
      const updateResponse = await fetch(`${BASE_URL}/parsed-documents/${firstDoc.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          examName: 'UPSC Civil Services Examination 2025 - Updated',
          confidence: 0.95
        })
      });
      ...
      const filterResponse = await fetch(`${BASE_URL}/parsed-documents?examType=upsc&limit=5`);
      ...
      console.log('\n6️⃣ Testing POST /api/parsed-documents (manual create)...');
      const createResponse = await fetch(`${BASE_URL}/parsed-documents`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          ...
          originalText: 'Manual test entry',
          confidence: 1.0,
          documentCount: 1,
          method: 'manual',
      ...
      console.log('\n7️⃣ Testing DELETE /api/parsed-documents/[id]...');
      const deleteResponse = await fetch(`${BASE_URL}/parsed-documents/${createResult.id}`, {
        method: 'DELETE'
      });
    ...
    console.log('\n📊 CRUD API Summary:');
    console.log('   ✅ CREATE: POST /api/parsed-documents');
    console.log('   ✅ READ: GET /api/parsed-documents & GET /api/parsed-documents/[id]');
    console.log('   ✅ UPDATE: PUT /api/parsed-documents/[id]');
    console.log('   ✅ DELETE: DELETE /api/parsed-documents/[id]');
    console.log('   ✅ FILTER: Query parameters (userId, examType, examName, etc.)');
    console.log('   ✅ AUTO-SAVE: Text-to-JSON automatically saves to database');
